...
if (isset($_GET['model_name'])) {
    ...
    if (isset($_GET['delete_id'])) {
        ...
        $query_delete = "DELETE FROM images WHERE id = $delete_id AND device_id = $device_id";
    ...
    if (isset($_GET['update_id'])) {
        ...
        // If the user submitted the update form, process it
        if (isset($_POST['update'])) {
            if ($_FILES['image']['error'] == UPLOAD_ERR_OK) {
                ...
                $query_update_image = "UPDATE images SET image_name = ?, image_data = ? WHERE id = $update_id AND device_id = $device_id";
                $stmt = $conn->prepare($query_update_image);
                $stmt->bind_param('ss', $image_name, $image_data);
                $stmt->execute();
                echo "Image updated successfully!";
            } else {
                echo "Error uploading updated image!";
            }
        }

        // Fetch the existing image to show in the update form
        $query_image = "SELECT * FROM images WHERE id = $update_id AND device_id = $device_id";
        $result_image = $conn->query($query_image);
        $image = $result_image->fetch_assoc();

        echo '<h3>Update Image</h3>
    ...
    // Display images and provide the option to delete or update
    while ($image = $result_images->fetch_assoc()) {
        echo '<div class="image-container">
                ...
                <br>
                <a href="?model_name=' . $model_name . '&delete_id=' . $image['id'] . '" class="delete-button" onclick="return confirm(\'Are you sure you want to delete this image?\')">Delete</a>
                <a href="?model_name=' . $model_name . '&update_id=' . $image['id'] . '" class="update-button">Update</a>
              </div>';
    }
