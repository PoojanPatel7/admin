...
if (isset($_POST['delete'])) {
    ...
    $sql_delete = "DELETE FROM key_specs WHERE spec_id = ?";
...
$sql = "SELECT ks.*, dev.model_name 
        FROM key_specs ks 
        JOIN device dev ON ks.device_id = dev.device_id 
        LIMIT $limit OFFSET $offset";
$result = $conn->query($sql);

// Get total number of records for pagination
$sql_count = "SELECT COUNT(*) FROM key_specs";
...
<head>
    ...
    <title>Manage Key Specifications</title>
    <style>
        /* General Styles */
        body {
...
<body>
    ...
    <h2>Manage Key Specifications</h2>

    <!-- Key Specs Deletion Form -->
    <form method="POST">
        <label for="spec_id">Select Key Spec Record:</label>
        <select name="spec_id" id="spec_id">
            ...
            while ($row = $result->fetch_assoc()): ?>
                <option value="<?= $row['spec_id']; ?>">
                    <?= $row['model_name']; ?> - <?= $row['Processor']; ?> (<?= $row['RAM']; ?> RAM)
                </option>
            <?php endwhile; ?>
        </select>
    ...
    <h3>Key Specifications Records</h3>
    <table>
        <thead>
            <tr>
                <th>Spec ID</th>
